...
\subsection{Varieties and rational maps}
  ...
  \begin{definition}
    ...
    \begin{itemize}
      \item a scheme $X$ projective and flat over $S$ such that every fiber $X_s$ with $s \in S$ is a variety over $\kappa(s)$;
...
\subsection{Siu's inequality and dynamical degrees}
  ...
  First we list some form of Siu's inequality which will be used in the proof of the main theorem also in the proof of the properties of dynamical degrees.

  Then original version of Siu's inequality is the following.
  \begin{theorem}[{Siu's criterion, ref. \cite[Theorem 2.2.15]{Laz04a}}]\label{thm:Siu_inequality}
    Let $X$ be a projective variety of dimension $d$ and $L,M$ be nef divisors on $X$.
    Suppose that $M$ is big, then 
    \[ M - \varepsilon \frac{M^d}{L \cdot M^{d-1}} L \]
