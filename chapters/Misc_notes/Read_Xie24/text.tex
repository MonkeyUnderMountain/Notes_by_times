...
\subsection{Estimation}
  ...
  The main goal is to find some lower bound $\beta_m \leq \lambda_i$ of $\lambda_i$ such that $\beta_m \to \lambda_i$ as $m \to \infty$ and $\beta_m$ depends only on some certain intersection numbers of line bundles of form $(f^m)^* L$.
  % the intersection numbers of $L_{2m}, L_m, L$.

  Set 
  \[ L_m := (f^m)^* L, \quad M_m(t) := L_{2m} + t^m L. \]
  We will try to estimate the ratio
  \[ \Gamma_t(m) := \Gamma_{X,L;t}(m) := \frac{M_m(t)^{d}}{M_m(t)^{d-1} \cdot L_m} = \frac{(L_{2m} + t^m L)^{d}}{(L_{2m} + t^m L)^{d-1} \cdot L_m}. \]
  ...
  and $a_m \asymp_e b_m$ if $a_m \preccurlyeq_e b_m$ and $a_m \succcurlyeq_e b_m$.\Yang{}
  % \Yang{}
  % \[ a_m \precnsim b_m \quad \text{if} \quad \liminf_{m \to \infty} \left(\frac{a_m}{b_m}\right)^{1/m} < 1. \]
  % $a_m \approx b_m$ if there exists some constant $C > 0$ such that $C^{-1} a_m \leq b_m \leq C a_m$ for every $m$.\Yang{}

  Note that 
  \[ \Gamma_t(m) \succcurlyeq_e  a_m \implies M_m(t) - a_m L_m \text{ is big for sufficiently large } m. \]
