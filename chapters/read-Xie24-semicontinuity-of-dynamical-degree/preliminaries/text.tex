...
\paragraph{Varieties}
Throughout this note, all schemes are (essentially) of finite type and separated over a field or a noetherian integral scheme.
...
Let $\Gamma_f \subset X \times Y$ be the scheme-theoretic image (see \cite[Tag 01R5]{Stacks}) of $(\id_U, f): U \to X \times Y$, where $U \subset X$ is the dense open subset where $f$ is defined.
...
\paragraph{Families of dominant rational maps}
...
\paragraph{Pullback by rational maps}
...
Let $f: \frakX \to \frakY$ be a dominant rational map between varieties of the same dimension.
By choosing models making $f: \frakX \to \frakY$ well-defined, we see that $f^*$ is still a ring homomorphism and the projection formula for intersection numbers is still valid for pullback by rational maps.
Explicitly, for $\alpha \in \CH^i(\frakY)$ and $\beta \in \CH^{j}(\frakX)$, we have
\begin{itemize}
	\item $f^*(\alpha \cdot \beta) = f^* \alpha \cdot f^* \beta \in \CH^{i+j}(\frakX)$;
	\item if $i+j = d$, then $f^* \alpha \cdot f^* \beta = (\deg f) \cdot (\alpha \cdot \beta) \in \bbZ$.
\end{itemize}
Here $\deg f$ is the degree of the field extension $\fkk(X)/\fkk(Y)$ induced by $f$.
...
\paragraph{Intersection with subvarieties}
...
\begin{corollary}\label{cor:generalized-Siu-inequality-restricted-to-hyperplane-sections}
...
Since \cref{thm:generalized_Siu_inequality,cor:generalized-Siu-inequality-restricted-to-hyperplane-sections} only involve finite many cocycles,
they can be generalized to $\bfb$-cocycles on birational models by taking a common defining model.

Now we can show the existence of dynamical degrees and their log-concavity.

\begin{definition}
	Let $f: X \dashrightarrow X$ be a dominant rational self-map on a projective variety $X$ of dimension $d$ and $H$ be an ample divisor on $X$.
	We define the \emph{$k$-th degree of $f$ with respect to $H$} as
	\[ \deg_{k,H}(f) \coloneqq (f^* H^k \cdot H^{d-k}) \in \bbZ. \]
	The \emph{$k$-th dynamical degree of $f$} is defined as
	The \emph{$k$-th dynamical degree of $f$} is defined as
	\[ \lambda_k(f) \coloneqq \lim_{n \to \infty} \left(\deg_{k,H}(f^n)\right)^{1/n}. \]
\end{definition}

Note by the projection formula, we have $\deg_{d,H}(f) = (\deg f) \cdot H^d$ and hence $\lambda_d(f) = \deg f$.
...
\end{proposition}
\begin{proof}
	Set $H_i = (f^i)^*H$ as $\bfb$-divisor on $\frakX$.
	Then we have
	\[ (H_{n+m}^k \cdot H^{d-k}) \cdot H_n^d \leq \binom{d}{k} \cdot (H_{n+m}^k \cdot H_n^{d-k}) \cdot (H_n^k \cdot H^{d-k}). \]
	That is,
	\[ (\deg_{k,H} f^{n+m}) \cdot (\deg f^n) \cdot H^d \leq \binom{d}{k} \cdot (\deg f^n) \cdot (\deg_{k,H} f^m) \cdot (\deg_{k,H} f^n). \]
	Set $a_i = \deg_{k,H} f^i$ and $C = C_d^k/H^d$.
	Then we have $a_{n+m} \leq C a_n a_m$.
	By mathematic analysis, the sequence $\{a_i^{1/i}\}$ converges to $\inf_i (Ca_i)^{1/i}$.
	Denote the limit by $\lambda_{k,H}(f)$.

	Let $L$ be another ample divisor on $X$.
	Then there exists a constant $a > 0$ and an ample divisor $D$ such that $L \sim aH + D$.
	Then we have
	\[ \deg_{k,L} f^n = (f^n)^* L^k \cdot L^{d-k} = (f^n)^* (aH + D)^k \cdot (aH + D)^{d-k} \geq a^d (f^n)^* H^k \cdot H^{d-k} = a^d \deg_{k,H} f^n. \]
	Taking $n$-th root and limit, we get $\lambda_{k,L}(f) \geq \lambda_{k,H}(f)$.
	By symmetry, we have $\lambda_{k,L}(f) = \lambda_{k,H}(f)$.
\end{proof}
