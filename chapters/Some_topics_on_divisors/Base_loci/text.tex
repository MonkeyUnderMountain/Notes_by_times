...
\subsection{Base loci of line bundles}
  ...
  \begin{example}\label{eg:support_of_Cartier_divisor_when_nonnormal_by_gluing_two_points}
    ...
    Since $\frakm_0 \cap \frakm_1 = (t, s(s-1), ts)$, we have 
    \[ A = \kk[t,u,v]/((v-t)v-t^2u). \]
    
    \Yang{}
    
    Then $A$ is a noetherian integral domain but not normal.
    and $X = \Spec A$.
    Then $X$ is obtained by gluing two points of $\bbA^2$ with coordinates $s,t$.
    Let $D$ be the divisor defined by $s$ on \Yang{}

    We describe $A$ more explicitly.
    Indeed, 
    \[ A = \kk[u,v,w]...\]
  ...
  %   \[ \frac{x}{y} \text{ on } U_y = X \setminus \{y=0\} \cong \Spec \kk\left[x,y,\frac{1}{y}\right], \quad \frac{yz}{x} \text{ on } U_x = X \setminus \{x=0\} \cong \Spec \kk\left[x,y,z,\frac{1}{x}\right]/(x^2 - y^2 z). \]
  %   On the overlap $U_x \cap U_y$, we have $\frac{yz}{x} = \frac{x}{y}$, so the above data defines a Cartier divisor on $X$.
  ...
  \begin{theorem}\label{thm:semi-ample_fibration}
    ...
    % When $D$ is on a projective variety $Y$ and $X = Y \setminus \bfB(D)$, it is also called the \emph{Iitaka fibration} of $D$.
     \Yang{}
  \end{theorem}
  \begin{proof}
    ...
    Let $\phi_D: X \to \bbP^n$ be the morphism induced by $s_0, \dots, s_n$ (\cite[Chapter II, Theorem 7.1]{Har77}) and $Y$ be relative normalization (\Yang{}) of $X$ in $\bbP^n$.
    Then $\phi_D$ factors as $X \xrightarrow{f} Y \xrightarrow{g} \bbP^n$ with $g$ finite and $f$ a fibration.
    Let $A = g^* \calO_{\bbP^n}(1)$, then $A$ is ample since $g$ is finite, and $D \sim f^* A$ since $\phi_D^* \calO_{\bbP^n}(1) \sim D$.
    Hence we get the existence of the desired fibration.

    Let $f':X \to Y'$ and $A'$ on $Y'$ be another such fibration and ample divisor.
    Replace $D$ be a sufficiently divisible multiple, we may assume that both $A$ and $A'$ are very ample.
  ...
  When $X$ is projective, we can detect the dimension of the image of the Iitaka fibration by some asymptotic behavior of the dimension of global sections of multiples of $D$.
