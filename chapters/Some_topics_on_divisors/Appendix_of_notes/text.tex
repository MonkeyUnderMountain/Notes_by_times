...
\subsection{Fibrations}
  ...
  \begin{proof}
    ...
    Assertion (b) follows from the universal property of the relative normalization (see \cref{prop:universal_property_of_relative_normalization}).
...
\subsection{Valuations in general settings}
  ...
  \begin{theorem}[{ref. \cite[Remark 1.14]{Vaquie-2006-valuations}}]
    Let $\nu: \KK^\times \to \Gamma_\nu$ be a valuation on $\KK/\kk$.
    Then we have 
    \[ \rank \nu \leq \rank_\bbQ \nu, \quad \rank_\bbQ \nu + \dim \nu \leq \trdeg \KK/\kk. \]
  \end{theorem}
  ...
  Note though the definition of $\rank \nu$ depends on $\calO_{\nu}$.\Yang{}

  \begin{definition}
    ideal of totally ordered abelian group \Yang{}
  \end{definition}

  \begin{proposition}
    \begin{enumerate}
      \item all ideals of $\calO_\nu$ forms a totally ordered set.
      \item there is $1:1$-correspondence;
      \item $\rank v = \dim \calO_\nu = \rank \Gamma$.
    \end{enumerate}
    \Yang{}
  \end{proposition}
  ...
  \begin{example}
    A valuation is of rank $1$ iff $\Gamma_v \subset \bbR$.
    \Yang{}
  \end{example}
  ...
  \begin{example}
    ...
    \[ \kk((\Gamma)) \coloneqq \left\{ (a_\gamma)_{\gamma \in \Gamma} \in \kk^{\Gamma} \;\middle|\; \Supp (a_\gamma) \subset \Gamma \text{ is well-order set (or satisfies DCC)} \right\}. \] 
    % \[ \kk((\Gamma)) \coloneqq \left\{ \sum_{\alpha \geq \alpha_0 \in \Gamma} a_\alpha x^\alpha \;\middle|\; \alpha_0 \in \Gamma, a_\alpha \in \kk \right\}. \]
    Here $x^\alpha$ is a formal symbol satisfying the relation $x^\alpha \cdot x^\beta = x^{\alpha+\beta}$.
