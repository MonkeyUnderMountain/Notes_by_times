...
\subsection{Varieties over non-closed fields}
  ...
  \begin{lemma}[{ref. \cite[\href{https://stacks.math.columbia.edu/tag/0G33}{Tag 0G33} and \href{https://stacks.math.columbia.edu/tag/054Q}{054Q}]{Stacks}}]
...
\subsection{Fibrations}
  ...
  \begin{proof}
    ...
    Now we prove (c).
    Denote by $Y' \to Y$ the relative normalization of $Y$ in $X$.
    Since $X \to Y'$ is dominant and $X$ is irreducible, $Y'$ is also irreducible.
    Since locally $\calO_{Y'}$ is a subalgebra of $f_* \calO_X$, $Y'$ is also reduced.
    Noetherian and excellent properties of $Y'$ follow from the fact that $Y' \to Y$ is finite.
    ...
    % I claim that $Y'_\kkk$ is the relative normalization of $Y_\kkk$ in $X_\kkk$.
    % Then the conclusion follows directly.
    % By replace $Y$ by an affine open subset and $X$ by $\Spec \calO_X(X)$, we may assume that $Y = \Spec A$ and $X = \Spec B$ are affine.
    % Then $Y' = \Spec A'$, where $A'$ is the integral closure of $A$ in $B$.
    % Let $C \subset B_\kkk$ be the integral closure of $A_\kkk$ in $B_\kkk$.
    % For any $f \in C$, we show that $f \in A'_\kkk$.
    % First for any finite extension $\bfl/\kk$, if $\alpha \in B_\bfl$ is integral over $A_\bfl$ with minimal polynomial $x^n + a_{n-1} x^{n-1} + \cdots + a_0$,  
    % There exists a finite extension $\kk'/\kk$ such that $f \in B \ten_\kk \kk'$, since $B$ is of finite type over $\kk$.
    % Then $f$ is integral over $A \ten_\kk \kk'$, 
    % \Yang{}
  \end{proof}
  ...
  \begin{proposition}\label{prop:equivalent_definitions_of_fibrations}
    ...
    \begin{enumerate}
      ...
      \end{itemize}
      \item \begin{itemize}[leftmargin=1em,nosep]
        \item if $X$ is normal, then (fibration) $\implies$ (alg. closed); 
        \item if $Y$ is normal, then (alg. closed) $\implies$ (fibration);
      \end{itemize}
      \item \begin{itemize}[leftmargin=1em,nosep]
      ...
      \item \begin{itemize}[leftmargin=1em,nosep]
        ...
        \item if $f$ is separable and $X$ is normal, then (connected) $\implies$ (fibration).
      \end{itemize}
    \end{enumerate}
    In particular, if $X$ and $Y$ are normal proper varieties and $\characteristic \kk = 0$, then all the above conditions are equivalent.
     \Yang{}
  \end{proposition}
  \begin{proof}
    \hspace{1em}
    ...
    The assertion (a) follows from the definition of the relative normalization and the fact that $f_* \calO_X$ is a quasi-coherent $\calO_Y$-algebra.\Yang{}
    ...
    When $Y$ is normal, for any affine open subset $U \subset Y$, $\calO_Y(U)$ is integral closed in $\fkk(Y)$ and $\fkk(Y)$ is algebraically closed in $\fkk(X)$, so $\calO_Y(U)$ is integral closed in $\fkk(X)$ and hence in $\calO_X(f^{-1}(U))$.
    ...
    \end{step}
     ...
     For (c), the assertion (c) $\implies$ (fibration) when $Y$ is normal follows from (b).
  ...
  The following examples shows that some assumptions in \cref{prop:equivalent_definitions_of_fibrations} are necessary.
